...
- Mermaid.js記法、Vega/Vega-Lite仕様（```vega / ```vega-lite フェンス）の図をレンダリング（大きな図は「⛶ 別ウィンドウで開く」リンクから図だけを別タブで拡大表示可能）
- サイドペインにファイルツリーを表示（大量のファイルがあるディレクトリを開いた場合は一部を省略して事故を防止）
- Markdown内のリンクをたどってページ遷移
- 画像ファイルはそのままプレビュー、それ以外のファイルはソースを表示。Markdown内の画像はクリックすると別タブで拡大表示可能
